...
row: the reference Model code plus its input generators. Level 0 is the
custom task source (data/custom.parquet); levels 1-4 are KernelBench.
"""
...
def load_task(level: int, problem_id: int) -> Task:
    parquet = DATA / ("custom.parquet" if level == 0 else f"level_{level}.parquet")
    if not parquet.is_file():
        raise FileNotFoundError(f"dataset missing: {parquet}")
    df = pd.read_parquet(parquet)
    rows = df[df.problem_id == problem_id]
    if rows.empty:
        raise KeyError(f"no problem_id={problem_id} in {parquet.name}")
    row = rows.iloc[0]
    return Task(
